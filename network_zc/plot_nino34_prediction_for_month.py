...
nino34_from_data = index_calculation.get_nino34_from_data(file_num, month-time_step)
for di, directly_month in enumerate(directly_months):
    ...
    for start_month in range(file_num-prediction_month*directly_month,
        ...
        if not is_sequence:
            ...
            for i in range(time_step):
                predict_data[0][i] = file_helper_unformatted.read_data_sstaha(start_month+i)
    ...
    x = np.linspace(file_num, file_num + month, month + 1 - time_step)
    plt.subplot(221+di)
    plt.plot(x, nino34, 'b')
