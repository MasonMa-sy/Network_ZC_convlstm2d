...
class DSSIMObjectiveCustom():
    ...
    def __call__(self, y_true, y_pred):
        ...
        # subtract the minimum value of two patterns to remove negative values -custom
        min_true = KC.min(y_true)
        min_pred = KC.min(y_pred)
        min_tp = KC.minimum(min_true, min_pred)
        y_true = y_true - min_tp
        y_pred = y_pred - min_tp

        # modify the strides to [1,1] -custom
        patches_pred = KC.extract_image_patches(y_pred, kernel, [1, 1], 'valid', self.dim_ordering)
        patches_true = KC.extract_image_patches(y_true, kernel, [1, 1], 'valid', self.dim_ordering)
