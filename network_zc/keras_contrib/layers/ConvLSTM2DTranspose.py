...
from keras.layers.recurrent import _generate_dropout_mask, RNN, _standardize_args
from keras.legacy import interfaces
from keras.utils import conv_utils
from keras import backend as K, activations, initializers, regularizers, constraints
import tensorflow as tf
from keras.utils.generic_utils import transpose_shape, has_arg, to_list


class ConvRNN2DTranspose(RNN):
    ...
    def __init__(self, cell,
        ...
        super(ConvRNN2DTranspose, self).__init__(cell,
                                                 return_sequences,
                                                 return_state,
                                                 go_backwards,
                                                 stateful,
    ...
    def compute_output_shape(self, input_shape):
        ...
        if cell.data_format == 'channels_first':
            ...
            rows = input_shape[2]
            cols = input_shape[3]
        rows = conv_utils.deconv_length(rows,
                                        kernel_size=cell.kernel_size[0],
                                        padding=cell.padding,
                                        stride_size=cell.strides[0],
                                        dilation=cell.dilation_rate[0],
                                        output_padding=None)
        cols = conv_utils.deconv_length(cols,
                                        kernel_size=cell.kernel_size[1],
                                        padding=cell.padding,
                                        stride_size=cell.strides[1],
    ...
    def __call__(self, inputs, initial_state=None, constants=None, **kwargs):
        ...
        if initial_state is None and constants is None:
            return super(ConvRNN2DTranspose, self).__call__(inputs, **kwargs)
        ...
        if K.is_keras_tensor(additional_inputs[0]):
            ...
            output = super(ConvRNN2DTranspose, self).__call__(full_input, **kwargs)
...
class ConvLSTM2DTransposeCell(Layer):
    ...
    def __init__(self, filters,
                 ...
                 recurrent_dropout=0.,
                 **kwargs):
        super(ConvLSTM2DTransposeCell, self).__init__(**kwargs)
    ...
    def input_conv(self, x, w, b=None, padding='valid'):
        # H1 = (x.shape[1] - 1) * self.strides[0] + w.shape[0]
        # H2 = (x.shape[2] - 1) * self.strides[1] + w.shape[1]
        rows = conv_utils.deconv_length(x.shape[1],
                                        kernel_size=self.kernel_size[0],
                                        padding=self.padding,
                                        stride_size=self.strides[0],
                                        dilation=self.dilation_rate[0],
                                        output_padding=None)
        cols = conv_utils.deconv_length(x.shape[2],
                                        kernel_size=self.kernel_size[1],
                                        padding=self.padding,
                                        stride_size=self.strides[1],
                                        dilation=self.dilation_rate[1],
                                        output_padding=None)
        output_shape = (-1, rows, cols, w.shape[3])
    ...
    def recurrent_conv(self, x, w):
        output_shape = (-1, x.shape[1], x.shape[2], w.shape[3])
        conv_out = K.conv2d_transpose(x, w, strides=(1, 1),
                                      output_shape=output_shape,
                                      padding='same',
                                      data_format=self.data_format)
        return conv_out
    ...
    def get_config(self):
        ...
        base_config = super(ConvLSTM2DTransposeCell, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))


class ConvLSTM2DTranspose(ConvRNN2DTranspose):
    """Convolutional LSTM.
    ...
    def __init__(self, filters,
                 ...
                 recurrent_dropout=0.,
                 **kwargs):
        cell = ConvLSTM2DTransposeCell(filters=filters,
        ...
        super(ConvLSTM2DTranspose, self).__init__(cell,
    ...
    def call(self, inputs, mask=None, training=None, initial_state=None):
        return super(ConvLSTM2DTranspose, self).call(inputs,
                                                     mask=mask,
                                                     training=training,
    ...
    def get_config(self):
        config = {'filters': self.filters,
                  ...
                  'dropout': self.dropout,
                  'recurrent_dropout': self.recurrent_dropout}
        base_config = super(ConvLSTM2DTranspose, self).get_config()
